...
const titleData = {
  ...
  subtitleText:
    "Prethodnih godina smo organizovali mnoge dogadjaje na Zlatiboru i na Fakultetu. Ovde je rezime ovog događaja!",
  colorText: "[]",
...
function GalleryEvent({ subDetails, whiteBackground }) {
  const imgs =
    (subDetails?.images?.length &&
      subDetails?.images.map((i) => ({ ...i, src: i.url }))) ||
    images;
  const randomImgs = [
    ...imgs.sort(() => Math.random() - Math.random()).slice(0, 14),
  ];
